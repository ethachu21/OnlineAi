"""
DeepSeek Handler
Handles API calls to DeepSeek AI models
"""
...
class DeepSeekHandler(GenerationHandler):
    """Handler for DeepSeek AI models with multiple model support"""
    
    def __init__(self):
        self.name = "deepseek"
        self.provider_config = config.get_provider('deepseek')
        ...
        self.base_url = self.provider_config.base_url if self.provider_config else "https://api.deepseek.com/v1/chat/completions"
    ...
    def is_available(self) -> bool:
        """Check if DeepSeek handler is properly configured"""
    ...
    def _supports_system_role(self) -> bool:
        """DeepSeek supports system roles natively"""
        return True
    ...
    def generate_stream(self, prompt: str, messages: List[Dict] = None):
        ...
        if not self.is_available():
            yield {
                "success": False,
                "error": "DeepSeek handler not properly configured - no API keys available",
                "provider": self.name
            }
            return
        ...
        for model_attempt in range(len(self.models)):
            ...
            while key_attempts < max_key_attempts:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning(f"DeepSeek {current_model} key {self.current_key_index + 1} failed: HTTP {response.status_code}")
                        key_attempts += 1
                    ...
                    logger.warning(f"DeepSeek {current_model} key {self.current_key_index + 1} failed: {str(e)}")
                    key_attempts += 1
            ...
            if model_attempt < len(self.models) - 1:
                if self._rotate_model():
                    logger.info(f"All DeepSeek keys failed for {current_model}, trying {self.models[self.current_model_index]}")
        
        # All models and keys failed
        yield {
            "success": False,
            "error": f"All DeepSeek models and keys failed. Models tried: {models_tried}",
            "provider": self.name,
    ...
    def generate(self, prompt: str, messages: List[Dict] = None) -> Dict[str, Any]:
        """Generate a response using DeepSeek API with model and key rotation"""
        if not self.is_available():
            return {
                "success": False,
                "error": "DeepSeek handler not properly configured - no API keys available",
