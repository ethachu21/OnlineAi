"""
Meta Handler
Handles API calls to Meta Llama models
"""
...
class MetaHandler(GenerationHandler):
    """Handler for Meta Llama models with multiple model support"""
    
    def __init__(self):
        self.name = "meta"
        self.provider_config = config.get_provider('meta')
        ...
        self.base_url = self.provider_config.base_url if self.provider_config else "https://api.llama-api.com/chat/completions"
    ...
    def is_available(self) -> bool:
        """Check if Meta handler is properly configured"""
    ...
    def _supports_system_role(self) -> bool:
        """Meta Llama supports system roles through jailbreak techniques"""
        return False  # Will use jailbreak method
    
    def generate_stream(self, prompt: str, messages: List[Dict] = None):
        """Generate a streaming response using Meta API"""
        if not self.is_available():
            yield {
                "success": False,
                "error": "Meta handler not properly configured - no API keys available",
                "provider": self.name
            }
            return
        ...
        for model_attempt in range(len(self.models)):
            ...
            while key_attempts < max_key_attempts:
                ...
                try:
                    ...
                    # Build the messages array for Meta
                    meta_messages = []
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning(f"Meta {current_model} key {self.current_key_index + 1} failed: HTTP {response.status_code}")
                        key_attempts += 1
                    ...
                    logger.warning(f"Meta {current_model} key {self.current_key_index + 1} failed: {str(e)}")
                    key_attempts += 1
            ...
            if model_attempt < len(self.models) - 1:
                if self._rotate_model():
                    logger.info(f"All Meta keys failed for {current_model}, trying {self.models[self.current_model_index]}")
        
        # All models and keys failed
        yield {
            "success": False,
            "error": f"All Meta models and keys failed. Models tried: {models_tried}",
            "provider": self.name,
    ...
    def generate(self, prompt: str, messages: List[Dict] = None) -> Dict[str, Any]:
        """Generate a response using Meta API with model and key rotation"""
        if not self.is_available():
            return {
                "success": False,
                "error": "Meta handler not properly configured - no API keys available",
            ...
            while key_attempts < max_key_attempts:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        return {
                            "success": False,
                            "error": "No valid response from Meta",
                            "provider": self.name,
                            "model": current_model,
