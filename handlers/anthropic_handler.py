...
class AnthropicHandler(GenerationHandler):
    """Handler for Anthropic Claude models with multiple model support"""
    
    def __init__(self):
        self.name = "anthropic"
        self.provider_config = config.get_provider('anthropic')
        ...
        self.base_url = self.provider_config.base_url if self.provider_config else "https://api.anthropic.com/v1"
    ...
    def is_available(self) -> bool:
        """Check if Anthropic handler is properly configured"""
    ...
    def generate_stream(self, prompt: str, messages: List[Dict] = None, use_streaming_timeout: bool = False):
        ...
        if not self.is_available():
            yield {
                "success": False,
                "error": "Anthropic handler not properly configured - no API keys available",
                "provider": self.name
            }
            return
        ...
        for model_attempt in range(len(self.models)):
            ...
            while key_attempts < max_key_attempts:
                ...
                try:
                    ...
                    logger.warning(f"Anthropic {current_model} key {self.current_key_index + 1} failed: {str(e)}")
                    key_attempts += 1
            ...
            if model_attempt < len(self.models) - 1:
                if self._rotate_model():
                    logger.info(f"All Anthropic keys failed for {current_model}, trying {self.models[self.current_model_index]}")
        
        # All models and keys failed
        yield {
            "success": False,
            "error": f"All Anthropic models and keys failed. Models tried: {models_tried}",
            "provider": self.name,
    ...
    def generate(self, prompt: str, messages: List[Dict] = None) -> Dict[str, Any]:
        """Generate a response using Anthropic API with model and key rotation"""
        if not self.is_available():
            return {
                "success": False,
                "error": "Anthropic handler not properly configured - no API keys available",
            ...
            while key_attempts < max_key_attempts:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if 'content' in json_data and len(json_data['content']) > 0:
                            content_block = json_data['content'][0]
                        ...
                        return {
                            "success": False,
                            "error": "No valid response from Anthropic",
                            "provider": self.name,
                            "model": current_model,
